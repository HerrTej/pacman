...
class Game(object):
    ...
    def run(self):
        ...
        with open("prueba.arff", "a") as file:
            print("Tamaño", os.path.getsize("prueba.arff"))
            if os.path.getsize("prueba.arff") == 0:
            ...
            while not self.gameOver:
                ...
                if self.catchExceptions:
                    try:
                        ...
                        action = agent.getAction(previous_state)
                        self.state = self.state.generateSuccessor(agentIndex, action)
                        reward = self.state.getScore() - previous_state.getScore()
                        if reward <= 0:
                            reward = 0
                        if agentIndex == 0:
